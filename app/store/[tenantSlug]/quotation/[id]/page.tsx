...
export default function StoreQuotationDetailPage({
  ...
  const load = () => {
    const url = token ? `/api/quotes/${id}?token=${token}` : `/api/quotes/${id}`;
    fetch(url).then(r => r.ok ? r.json() : null).then(q => setQuote(q)).finally(() => setLoading(false));
  };
  ...
  const setStatus = async (status: string, extra?: Record<string, unknown>) => {
    setUpdating(true);
    try {
      // Token-based access uses the token in a POST body; authenticated access uses the cookie.
  ...
  if (!quote) return <Box p={20} textAlign="center" color="gray.400">Quote not found or link has expired.</Box>;
  ...
  return (
    <Box maxW="960px" mx="auto" px={{ base: 4, md: 6 }} py={6}>
      ...
      {quote.status === 'ACCEPTED' && (
        <Box p={4} bg="green.50" rounded="xl" border="1px solid" borderColor="green.200" textAlign="center">
          <Text fontWeight={700} color="green.700">✅ Quote Accepted — your order is being processed.</Text>
        </Box>
      )}
    </Box>
  );
