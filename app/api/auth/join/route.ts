...
import { TenantStatus, MembershipRole, UserStatus, Prisma } from '@prisma/client';
import * as bcrypt from 'bcryptjs';

// POST /api/auth/join — tenant self-registration
// Creates tenant(PENDING_APPROVAL) + admin user + TENANT_ADMIN membership
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const tenant = await tx.tenant.create({
        data: { slug, name: companyName, legalName, email, phone, gstNumber, industry, status: TenantStatus.PENDING_APPROVAL },
      });
