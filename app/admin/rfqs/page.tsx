...
import {
  ...
  Separator, SimpleGrid, Textarea, Field, Input,
          ...
          <Box overflowX="auto">
            <Box as="table" w="full" style={{ borderCollapse: 'collapse', minWidth: '700px' }}>
              <Box as="thead" bg="gray.50" borderBottom="1px solid" borderColor="gray.100">
                <Box as="tr">
                  {['RFQ Number', 'Customer', 'Project', 'Items', 'Date', 'Assigned To', 'Status', 'Actions'].map(h => (
              ...
              <Box as="tbody">
                {paginated.map(rfq => (
                  <Box as="tr" key={rfq.id} borderTop="1px solid" borderColor="gray.50" _hover={{ bg: 'gray.50' }} transition="all 0.1s">
                    <Box as="td" px={4} py={3}>
                    ...
                    <Box as="td" px={4} py={3}>
                      ...
                      <Text fontSize="xs" color="gray.500">{rfq.companyName}</Text>
                    </Box>
                    <Box as="td" px={4} py={3}>
                      <Text fontSize="sm" color="gray.700">{rfq.projectName}</Text>
                    </Box>
                    <Box as="td" px={4} py={3}>
                      <Text fontSize="sm" color="gray.700">{rfq.items.length}</Text>
                    </Box>
                    <Box as="td" px={4} py={3}>
                      <Text fontSize="sm" color="gray.600">{rfq.createdAt}</Text>
                    </Box>
                    <Box as="td" px={4} py={3}>
                      <Text fontSize="xs" color={rfq.assignedTo ? 'gray.700' : 'gray.400'}>{rfq.assignedTo || 'Unassigned'}</Text>
                    </Box>
                    <Box as="td" px={4} py={3}>
                      <StatusBadge status={rfq.status} />
                    </Box>
                    <Box as="td" px={4} py={3}>
                      <Button size="xs" variant="outline" colorPalette="blue" rounded="md" onClick={() => openDetail(rfq)}>View</Button>
                    </Box>
                  </Box>
                ))}
              </Box>
          ...
          <DrawerBody py={4} overflowY="auto">
            {selectedRFQ && (
              <VStack gap={5} align="stretch">
                {/* Customer */}
                <Box>
                  <Text fontWeight={700} fontSize="sm" color="gray.600" mb={2} textTransform="uppercase" letterSpacing="wide">Customer</Text>
                  <SimpleGrid columns={2} gap={3}>
                    {[['Name', selectedRFQ.customerName], ['Company', selectedRFQ.companyName], ['Mobile', selectedRFQ.mobile], ['Email', selectedRFQ.email]].map(([l, v]) => (
                      <Box key={l}>
                        ...
                        <Text fontSize="sm" fontWeight={600}>{v}</Text>
                      </Box>
                    ))}
                  </SimpleGrid>
                </Box>
                ...
                {/* Project */}
                <Box>
                  <Text fontWeight={700} fontSize="sm" color="gray.600" mb={2} textTransform="uppercase" letterSpacing="wide">Project</Text>
                  <SimpleGrid columns={2} gap={3}>
                ...
                {/* Products */}
                <Box>
                  <Text fontWeight={700} fontSize="sm" color="gray.600" mb={2} textTransform="uppercase" letterSpacing="wide">Products ({selectedRFQ.items.length})</Text>
                  <VStack gap={2} align="stretch">
                    {selectedRFQ.items.map(item => {
                      const p = products.find(x => x.id === item.productId);
                      const b = brands.find(x => x.id === p?.brandId);
                      return (
                        <Flex key={item.productId} justify="space-between" bg="gray.50" rounded="lg" px={3} py={2}>
                          <Box>
                            <Text fontSize="sm" fontWeight={600}>{p?.name}</Text>
                            <Text fontSize="xs" color="gray.500">{b?.name} • {p?.sku}</Text>
                          </Box>
                          <Text fontWeight={700} fontSize="sm">×{item.quantity}</Text>
                        </Flex>
                      );
                    })}
                  </VStack>
                ...
                {/* Assign */}
                <Box>
                  <Text fontWeight={700} fontSize="sm" color="gray.600" mb={2} textTransform="uppercase" letterSpacing="wide">Assign Salesperson</Text>
                  <HStack gap={2}>
                    <select
                      value={assignee}
                      onChange={e => setAssignee(e.target.value)}
                      style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid #e2e8f0', background: 'white', fontSize: '14px', flex: 1 }}
                    >
                ...
                <Box>
                  ...
                  <VStack gap={2} align="stretch">
                    {selectedRFQ.status === 'New' && (
                      <Button colorPalette="blue" onClick={() => updateStatus(selectedRFQ, 'Under Review')}>Mark Under Review</Button>
                    )}
                    {selectedRFQ.status !== 'Quote Ready' && selectedRFQ.status !== 'Accepted' && (
                      <Button colorPalette="green" onClick={() => createQuote(selectedRFQ)}>Create Quote</Button>
                    )}
                    {!['Rejected', 'Expired'].includes(selectedRFQ.status) && (
                      <Button colorPalette="orange" variant="outline" onClick={() => updateStatus(selectedRFQ, 'Follow-Up')}>Mark Follow-Up</Button>
                    )}
                  </VStack>
                </Box>
                <Separator />
                {/* Timeline */}
                <Box>
                  <Text fontWeight={700} fontSize="sm" color="gray.600" mb={2} textTransform="uppercase" letterSpacing="wide">Timeline</Text>
                  <VStack gap={2} align="stretch">
                    {selectedRFQ.timeline.map((t, i) => (
                      <HStack key={i} gap={3} align="flex-start">
                        <Box w={2} h={2} rounded="full" bg="blue.400" mt={1.5} flexShrink={0} />
                        <Box>
                          <Text fontSize="xs" fontWeight={600}>{t.action}</Text>
                          <Text fontSize="xs" color="gray.500">{t.date} · {t.by}{t.note ? ` — ${t.note}` : ''}</Text>
                        </Box>
                      </HStack>
                    ))}
                  </VStack>
