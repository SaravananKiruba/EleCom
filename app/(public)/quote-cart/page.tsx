...
export default function QuoteCartPage() {
  ...
  useEffect(() => {
    if (cartItems.length === 0) { setProducts({}); return; }
    // Fetch each product individually — small carts, keeps API simple.
    Promise.all(cartItems.map(async i => {
      const list = await fetch(`/api/store/current/products?take=1&q=${encodeURIComponent(i.productId)}`)
        .then(r => r.json()).catch(() => null);
      // Fallback: fetch every product on the first page and match by id — cheap in demo scale
      return list;
    })).then(() => {
      // Use a bulk fetch via list to hydrate — simpler and correct.
  ...
  return (
    <Box maxW="1200px" mx="auto" px={{ base: 4, md: 6 }} py={6}>
      ...
      <Flex justify="space-between" align="center" mb={6} flexWrap="wrap" gap={3}>
        ...
        {cartItems.length > 0 && <Button variant="ghost" colorPalette="red" size="sm" onClick={clearAll}>Clear All</Button>}
      </Flex>
                  ...
                  <Box key={item.productId} bg="white" rounded="xl" p={4} border="1px solid" borderColor="gray.100" shadow="sm">
                    <Flex gap={4} align="center" flexWrap="wrap">
                      ...
                      <Box flex={1} minW="140px">
                        ...
                        <Text fontWeight={600} color="gray.800" fontSize="sm">{p?.name ?? item.productId}</Text>
                      ...
                      <HStack gap={3} flexShrink={0}>
                        ...
                        <Button size="sm" variant="ghost" colorPalette="red" onClick={() => remove(item.productId)}>✕</Button>
                      </HStack>
                    </Flex>
                  </Box>
                );
          ...
          <Box>
            <Box bg="white" rounded="xl" p={5} border="1px solid" borderColor="gray.100" shadow="sm" position={{ lg: 'sticky' }} top="80px">
              ...
              <VStack gap={3} align="stretch" mb={4}>
                <Flex justify="space-between"><Text fontSize="sm" color="gray.600">Products</Text><Text fontWeight={600}>{cartItems.length}</Text></Flex>
